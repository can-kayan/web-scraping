...
async function asianHandicap(browser, url, season) {
  const keywords = [
    "BET365",
    "BETFAIR",
    "PINNACLE",
  ...
  const start = tabLabels.map(async (item, c) => { 
    ...
    console.log(`🔄 Asian Handicap - ${tabLabels[c]} - Başladı`);
    ...
    console.log(
      `✅ Asian Handicap - ${tabLabels[c]} toplam indirilecek alan ${formattedThresholds.length}`
    );
        ...
        limit(async () => {
          ...
          try {
            ...
            for (let a = 0; a < arrows.length; a++) {
              ...
              const positionHint = a % 2 === 0 ? "one" : "two";
    ...
    await page.close();
    console.log(`✅ Asian Handicap - ${tabLabels[c]} - Tamamlandı`);
  });

  await Promise.all(start);
  const result = Object.values(firmMap);

  console.log(`✅ Asian Handicap Tüm işlemler tamamlandı`);
  return result;
}
