...
async function overUnder(browser, url, season) {
  const keywords = [
    "BET365",
    "BETFAIR",
    "PINNACLE",
  ...
  const urlsss = ["#over-under;2;", "#over-under;3;", "#over-under;4;"];
  const firmMap = {};
  ...
  const start = tabLabels.map(async (item, c) => {
    ...
    console.log(`🔄 Over/Under - ${tabLabels[c]} - Başladı`);
    ...
    console.log(
      `✅ Over/Under - ${tabLabels[c]} toplam indirilecek alan ${formattedThresholds.length}`
    );
    const limit = pLimit(1);
    // Threshold sayfaları sırayla işleniyor ama her page paralel
    await Promise.all(
        ...
        limit(async () => {
          ...
          try {
            ...
            for (let a = 0; a < arrows.length; a++) {
              ...
              const positionHint = a % 2 === 0 ? "over" : "under";
    ...
    await page.close();
    console.log(`✅ Over/Under - ${tabLabels[c]} - Tamamlandı`);
  });

  await Promise.all(start);
  const result = Object.values(firmMap);
  console.log(JSON.stringify(result, null, 2));
  console.log(`✅ Over/Under Tüm işlemler tamamlandı`);
  return result;
}
